...
# Create description based on latex file
...
def my_function(class_code):
  ...
  for section in sections:
    ...
    section_description = generate_description_for_section(section_content)
    output += f"************ SECTION: {section_name}\n{section_description}\n\n"

  #  print narrative to file
  description_file_name = os.path.join(COURSE_PATH, class_name, class_code + '_description' + '.txt')
  print('Writing to ', description_file_name)
  with open(description_file_name, 'w', encoding='utf-8') as file:
    file.write(output)
